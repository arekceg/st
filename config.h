...
static char *openticketcmd[] = { "/bin/sh", "-c",
    "sed 's/.*│//g' | grep -aEo '([A-Z]+-[0-9]+\\S*)' | uniq | rofi -dmenu -i -p 'Open which ticket?' -l 10 | grep -aEo '([A-Z]+-[0-9]+)' | sed 's/^/ https:\\/\\/helpdesk\\.bravurasolutions\\.com\\/browse\\//' | xargs -r xdg-open",
    "externalpipe", NULL };
...
static Shortcut shortcuts[] = {
	...
	{ ALTMOD,               XK_t,           externalpipe,   {.v = openticketcmd } },
